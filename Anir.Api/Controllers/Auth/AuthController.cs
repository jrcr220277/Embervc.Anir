...
public class AuthController : ControllerBase
{
    ...
    public async Task<ActionResult<ProcessResponse<bool>>> UpdateProfile([FromBody] UpdateProfileRequest request)
    {
        try
        {
            ...
            // Guardar ID viejo para posible limpieza
            int? oldImageId = user.ImageFileId;

            // Actualizar datos
            user.ImageFileId = request.ImageFile?.Id; // Asegúrate que tu Request tenga int? ImageFileId
            ...
            // Si cambió la foto de perfil, borramos la antigua del disco
            if (oldImageId.HasValue && oldImageId != user.ImageFileId)
            {
                try { await _fileStorage.DeleteAsync(oldImageId.Value); }
                catch (Exception ex) { _logger.LogWarning(ex, "No se pudo eliminar imagen antigua {Id}", oldImageId); }
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return Ok(ProcessResponse<LoginResponse>.Success(response, "Inicio de sesión exitoso."));
    }
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (!result.Succeeded)
            {
                ...
                _logger.LogWarning("Register: Error creando usuario {Email}: {Errors}", request.Email, string.Join("; ", result.Errors.Select(e => e.Description)));
            ...
            if (!roleResult.Succeeded)
            {
                ...
                _logger.LogError("Register: Error asignando rol Usuario a {Email}: {Errors}", user.Email, string.Join("; ", roleResult.Errors.Select(e => e.Description)));
            ...
            return Ok(ProcessResponse<RegisterResponse>.Success(response, "Usuario registrado correctamente. Revise su correo para confirmar la cuenta."));
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> ConfirmEmail([FromQuery] string userId, [FromQuery] string token)
    {
        try
        {
            ...
            if (!result.Succeeded)
            {
                _logger.LogWarning("ConfirmEmail: Token inválido o expirado para {Email}. Errores: {Errors}", user.Email, string.Join("; ", result.Errors.Select(e => e.Description)));
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            if (!ModelState.IsValid)
            {
                var validation = ModelState.Where(x => x.Value?.Errors.Any() == true).ToDictionary(kvp => kvp.Key, kvp => kvp.Value!.Errors.Select(e => e.ErrorMessage).ToArray());
            ...
            foreach (var validator in _userManager.PasswordValidators)
            {
                ...
                {
                    var errors = validationResult.Errors.ToDictionary(e => e.Code, e => new[] { e.Description });
                    _logger.LogWarning("ChangePassword: Nueva contraseña no cumple la política para {UserId}: {Errors}", userId, string.Join("; ", validationResult.Errors.Select(e => e.Description)));
            ...
            {
                var errors = result.Errors.ToDictionary(e => e.Code, e => new[] { e.Description });
                _logger.LogWarning("ChangePassword: Falló el cambio de contraseña para {UserId}: {Errors}", userId, string.Join("; ", result.Errors.Select(e => e.Description)));
    ...
    public async Task<ActionResult<ProcessResponse<bool>>> ResetPassword([FromBody] ResetPasswordRequest request)
    {
        try
        {
            ...
            try
            {
                decodedEmail = System.Text.Encoding.UTF8.GetString(WebEncoders.Base64UrlDecode(request.Email));
            }
            catch
            {
            ...
            try
            {
                decodedToken = System.Text.Encoding.UTF8.GetString(WebEncoders.Base64UrlDecode(request.Token));
            }
            catch
            {
            ...
            {
                var errors = result.Errors.ToDictionary(e => e.Code, e => new[] { e.Description });
                _logger.LogWarning("ResetPassword: Error restableciendo contraseña para {Email}: {Errors}", decodedEmail, string.Join("; ", result.Errors.Select(e => e.Description)));
    ...
    private UserResponse MapUser(ApplicationUser user, IList<string> roles)
    {
        ...
        return new UserResponse
        {
            ...
            // Armado parcial sin ir a la BD (Rápido para Login/Me)
            ImageFile = user.ImageFileId.HasValue
                ? new FileResponse
                {
                    Id = user.ImageFileId.Value,
                    Url = $"{Request.Scheme}://{Request.Host}/api/files/{user.ImageFileId.Value}"
                }
                : null,
